...
# Normalize `r` to owner/repo@ref, discarding an action subpath.
# Return "" for local or invalid references.
...
# Return a comparison key for `r` by folding case only on its owner/repository
# segment. GitHub resolves owner and repository names case-insensitively, and
...
FNR == 1 { wf = FILENAME }
{
  ...
  if (match(line, /^[[:space:]]*-?[[:space:]]*uses:[[:space:]]*(.+)$/, m)) {
    ...
    gsub(/^["']|["']$/, "", raw)
    gsub(/[[:space:]]+$/, "", raw)
    if (raw ~ /^\$\//) next  # same-repository reference
    n = norm(raw)
